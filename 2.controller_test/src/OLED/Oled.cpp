...
// 定义页面成员个数，列表类页面列表行数初始化，必须初始化的参数
void ui_init()
{
...
/*
总的UI进程，也就是当前ui层级，会一直进入
调用逻辑：开机进入S_NONE，页面切换时调用一次S_LAYER_IN或S_LAYER_OUT，再调用四次S_FADE，弹窗只调用S_WINDOW
*/
void ui_proc()
{
    u8g2.sendBuffer();
